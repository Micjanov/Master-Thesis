...
\begin{table}[h]
    ...
    \hline
    1953 & Double helix model for DNA by Watson, Crick, Franklin~\cite{dnastruct}. Insulin sequenced by Sagner~\cite{insulin}\\
    \addlinespace[2ex]
    1958 & Central Dogma proposed by Crick~\cite{dogma}\\
    \addlinespace
    1962 & COMPROTEIN by Dayhoff~\cite{comprotein}\\
    \addlinespace[2ex]
    1965 & First protein database by Dayhoff~\cite{atlas}\\
    \addlinespace[2ex]
    1968 & Genetic code deciphered~\cite{codon}\\
    \addlinespace[2ex]
    1970 & Needleman \& Munsch's global alignment algorithm~\cite{global}\\
    \addlinespace[2ex]
    1977 & First full genome sequenced (Phi X174)~\cite{dnaseq}\\
    \addlinespace[2ex]
    1981 & Smith \& Waterman's local alignment algorithm~\cite{local}\\
    \addlinespace[2ex]
    1987 & FASTA for DNA and protein searches released~\cite{fasta}\\
    \addlinespace[2ex]
    1991 & Human Genome project initiated~\cite{hgp}\\
    \addlinespace[2ex]
    2003 & Human genome fully mapped~\cite{hgpc}\\
    \addlinespace[2ex]
    2005 & Next-gen sequencing technology publicly available~\cite{ngs}\\
    \addlinespace[2ex]
    2018 & AlphaFold introduced~\cite{alphafold}\\
    \hline
    \end{tabularx}
    \end{table}
