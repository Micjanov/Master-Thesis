...
\mint{Julia}|bitHDV(N::Int=10000) = bitrand(N)|
...
\end{minted}
Lastly, the procedure of equation~\ref{eqn:ex1} is repeated 10000 times because of the stochastic nature of these vectors. The results are illustrated in figure~\ref{fig:exm1}.
...
We see that we can retrieve a lot of information with most of the Hamming distances centering around 0.25. Notice that two completely random HDVs would have a distance close to 1 just by stochastics. A cosine similarity of 1 would mean that we retrieved all bits of A correctly, which is impossible in this model due to the consideration of noise. This result is not comparable to state-of-the-art accuracies but the calculations are very efficient nonetheless as all of these calculations were done in less than 2 seconds. This same experiment was done with random bipolar 10000-dimensional vectors and it performs slightly slower as expected but retained the accuracy (similar \textit{cosine similarity} peak around 0.5).
\subsection{Examples of hyperdimensional computing on datasets}
Now, the power of these simple operations will be demonstrated by applying them to a couple of datasets.
\subsubsection{Zoo animal classification}
...
\begin{equation}
    B = P + C + K
\end{equation} 
To classify an animal, its HDV can be compared to the HDVs of every class and the most similar vector is then assumed to be its class.